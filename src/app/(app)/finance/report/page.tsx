...
const PERIOD_LABELS: Record<string, string> = {
  '1M': 'Ultimi 30 giorni', '3M': 'Ultimi 3 mesi',
  '6M': 'Ultimi 6 mesi', '1A': 'Ultimi 12 mesi', 'MAX': 'Tutto il periodo',
}
const CHART_LEGEND = [
  { color: 'var(--success)', label: 'Entrate' },
  { color: 'var(--danger)', label: 'Uscite' },
  { color: 'var(--accent)', label: 'Investito' },
]
...
function monthLabel(ym: string) {
  const [, m] = ym.split('-')
  return MONTH_SHORT[parseInt(m, 10) - 1] ?? m
}
...
export default function ReportPage() {
  ...
  const kpis = [
    { label: 'ENTRATE TOTALI', value: income,   prev: yoyIncome   as number | null, count: periodTx.filter((t) => t.type === 'income').length,                                               sub: 'movimenti',             color: 'var(--success)', hl: true  },
    { label: 'USCITE TOTALI',  value: expenses, prev: yoyExpenses as number | null, count: periodTx.filter((t) => t.type === 'expense').length,                                              sub: 'incl. ricorrenti',      color: 'var(--danger)',  hl: false },
    { label: 'INVESTITO',      value: invested, prev: yoyInvested as number | null, count: periodTx.filter((t) => t.type === 'expense' && INVEST_CATS.has(t.category)).length,               sub: 'operazioni',            color: 'var(--accent)',  hl: false },
    { label: 'RISPARMIATO',    value: saved,    prev: null        as number | null, count: null,                                                                                              sub: `${savingsRate.toFixed(0)}% al netto degli investimenti`, color: deltaClass(saved) === 'pos' ? 'var(--success)' : 'var(--danger)', hl: false },
  ]

  // ── Performance panels ────────────────────────────────────────
  const perfPanels = [
    { title: 'Performance · best',  rows: bestRows,  avg: avgBest,  positive: true  },
    { title: 'Performance · worst', rows: worstRows, avg: avgWorst, positive: false },
  ]
  ...
  return (
    <div className="ledgernest-gap-5">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 20 }}>Report · {PERIOD_LABELS[period]}</div>
          <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>Riepilogo finanziario · {months.length} {months.length === 1 ? 'mese' : 'mesi'}</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button className="ledgernest-btn ledgernest-btn-ghost" style={{ fontSize: 12 }} onClick={exportCSV}>↓ Export CSV</button>
        </div>
      </div>
      ...
      <div className="ledgernest-grid-2">
        <div className="ledgernest-card">
          <div className="ledgernest-card-header">
            <span className="ledgernest-card-title">Entrate, uscite e investito</span>
        ...
        <div className="ledgernest-card">
          ...
          <div className="ledgernest-card-body" style={{ paddingTop: 8 }}>
            {savingsLine.length > 1 ? (
              ...
              <div className="ledgernest-empty">Dati insufficienti</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="ledgernest-grid-2">
        ...
        <div className="ledgernest-card">
          <div className="ledgernest-card-header">
            <div>
              <span className="ledgernest-card-title">Top merchant</span>
              <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2 }}>Dove hai speso di più · ultimi 3 mesi</div>
            </div>
          </div>
            ...
            <table className="ledgernest-table" style={{ fontSize: 12 }}>
              ...
              <tbody>
                {topMerchants.length === 0
                  ? <tr><td colSpan={5} style={{ textAlign: 'center', padding: 24, color: 'var(--text-tertiary)' }}>Nessun dato</td></tr>
                  : topMerchants.map((m) => {
        ...
        <div className="ledgernest-card">
          <div className="ledgernest-card-header">
            <div>
              <span className="ledgernest-card-title">Uscite per categoria</span>
              <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2 }}>Distribuzione · {PERIOD_LABELS[period].toLowerCase()}</div>
            </div>
          </div>
          {byCategory.length === 0 ? (
            <div className="ledgernest-card-body"><div className="ledgernest-empty">Nessuna spesa</div></div>
          ) : (
            <>
      ...
      {perfRows.length > 0 && (
        <div className="ledgernest-grid-2">
          {perfPanels.map(({ title, rows, avg, positive }) => (
            <div key={title} className="ledgernest-card">
              <div className="ledgernest-card-header">
                <div>
                  <span className="ledgernest-card-title">{title}</span>
                  <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2 }}>
                    Posizioni con {positive ? 'miglior' : 'peggior'} rendimento nel periodo
                  </div>
                </div>
                <span style={{
